...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 *        	}
	 * @return boolean array "type":"video",
	 *         "media_id":"mediaid",
	...
	public function deleteMassMessage($msg_id) {
		if (! $this->access_token && ! $this->checkAuth ())
			return false;
		$result = $this->http_post ( self::API_URL_PREFIX . self::MASS_DELETE_URL . 'access_token=' . $this->access_token, self::json_encode ( array (
				'msg_id' => $msg_id 
		) ) );
		if ($result) {
			$json = json_decode ( $result, true );
	 ...
	 * @param int $msg_id
	 *        	消息id
	 * @return boolean array "msg_id":201053012, //群发消息后返回的消息id
	...
	public function queryMassMessage($msg_id) {
		if (! $this->access_token && ! $this->checkAuth ())
			return false;
		$result = $this->http_post ( self::API_URL_PREFIX . self::MASS_QUERY_URL . 'access_token=' . $this->access_token, self::json_encode ( array (
				'msg_id' => $msg_id 
		) ) );
		if ($result) {
			$json = json_decode ( $result, true );
	 ...
	 *        	传入要转换的长url
	 * @return boolean string 成功则返回转换后的短url
	 */
	...
	/**
	 * 获取关注者详细信息 modify by zhangxinghong 2015年7月1日
	 * 
	...
	public function getUserInfoByAccessToken($access_token,$openid) {
		...
		$result = $this->http_get ( self::API_URL_PREFIX . self::USER_INFO_URL . 'access_token=' . $access_token . '&openid=' . $openid );
		if ($result) {
			$json = json_decode ( $result, true );
			if (isset ( $json ['errcode'] )) {
				$this->errCode = $json ['errcode'];
	 ...
	 * 获取多客服在线客服接待信息
	 *
	 * @return boolean array "kf_online_list": [
	 *         {
	...
	public function setKFHeadImg($account, $imgfile) {
		if (! $this->access_token && ! $this->checkAuth ())
			return false;
		$result = $this->http_post ( self::API_BASE_URL_PREFIX . self::CS_KF_ACCOUNT_UPLOAD_HEADIMG_URL . 'access_token=' . $this->access_token . '&kf_account=' . $account, array (
				'media' => '@' . $imgfile 
		), true );
		if ($result) {
			$json = json_decode ( $result, true );
	 ...
	 * 获得卡券的最新颜色列表，用于创建卡券
	 * 
	 * @return boolean array 微信卡券接口文档 的json格式
	 */
	 ...
	 * @return boolean array "errcode":0,
	 *         "errmsg":"ok",
